...
カット／テロップ（背景画像・アイコン・複数行のセット）／字幕（SRT）／
画像（一部だけ切り出して配置）／効果音・BGM／全画面と部分のぼかし／マーカー／
アンドゥ／mp4 書き出し／YouTube 用のサムネイル作り。

- **動画はどこにも送らない。** 読み込みも書き出しも、すべてブラウザの中で完結する
- **字幕は SRT で出す。** 画面に焼き込まず、日本語と英語の 2 本を書き出して YouTube に添付する。
  ミュート視聴者にも英語圏にも届く
...
- **サムネイルも作れる。** 動画の 1 コマか手持ちの画像を敷いて、
  同じテロップ機能で文字を載せ、1280×720 の画像として保存できる。
  シリーズ物なら前回の `.kiriko` から文字組みを取り込める
...
字幕（SRT）も同じ経路で回せる。`kiriko_transcribe` で書き起こし →
`kiriko_set_subtitles` に流し込んで下書きを作り → `kiriko_get_subtitles` の警告を見ながら整え →
`id` を指定して `en` だけ渡せば英訳も進められる。
