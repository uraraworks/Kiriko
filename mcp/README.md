...
- **開く URL を変えると、覚えているものは全部消えます。** 作業フォルダも、
  読み込んだ素材の場所も、ブラウザが URL（オリジン）ごとに分けて持っているためです。
  公開ページと `http://localhost:8901`、ポート番号違いは、それぞれ別ものとして扱われます。
  普段と違う URL で開くと「素材が読めない」ように見えますが、ファイルの置き場所の
  問題ではありません。**いつも同じ URL で開いてください。**
